...
from test_hook import EvaluationHook
...
flags.DEFINE_integer(
    'steps_per_eval', default=1251*15,
...
flags.DEFINE_string(
    'lr_decay_type', 'exponential', 'Learning rate decay policy')
...
flags.DEFINE_float(
    'decay_factor', default=0.97, help='Learning rate decay')

flags.DEFINE_float(
    'decay_epochs', default=2.4, help='Exponential decay period')
...
def mnasnet_model_fn(features, labels, mode, params):
  ...
  if is_training:
    ...
    learning_rate = mnasnet_utils.build_learning_rate(scaled_lr, global_step,
                                                      ...
                                                      lr_decay_type=FLAGS.lr_decay_type,
  ...
  #reader = pywrap_tensorflow.NewCheckpointReader('gs://imagenet_training/mbnetv2_2/model.ckpt-437898')
  #var_to_shape_map = reader.get_variable_to_shape_map()
  #m = {}
  #name_to_var = {}
  #for n in tf.global_variables():
  #  name_to_var[n.name[:-2]] = n
  #  print(n.name[:-2])
  #for key in sorted(var_to_shape_map):
  #  #if key == 'global_step':
  #  #  continue
  #  modified_key = key.replace('legrnet_full', 'legrnet_full_masked')
  #  modified_key = modified_key.replace('/conv2d', '/masked_conv2d')
  #  conv_id = re.findall(r'/conv2d(_\d+)?', key)
  #  if len(conv_id):
  #    if conv_id[0] != '':
  #      new_id = (int(conv_id[0][1:])-1)%2
  #      if new_id == 0:
  #        modified_key = modified_key.replace('/masked_conv2d{}'.format(conv_id[0]), '/masked_conv2d')
  #      else:
  #        modified_key = modified_key.replace('/masked_conv2d{}'.format(conv_id[0]), '/masked_conv2d_1')
  #  if 'masked_conv2d' in modified_key:
  #    modified_key = modified_key.replace('/le_gr_net_model', '')
  #  if not modified_key in name_to_var:
  #    pass #print('Not found {}'.format(modified_key))
  #  else:
  #    print('Found {}'.format(modified_key))
  #    m[key] = name_to_var[modified_key]
  #tf.train.init_from_checkpoint('gs://imagenet_training/mbnetv2_3/model.ckpt-437898', m)
  ...
  if FLAGS.mode == 'pred':
    ...
    for ckpt in evaluation.checkpoints_iterator(
      ...
      try:
        ...
        predictions = mnasnet_est.predict(input_fn=imagenet_eval.input_fn)
        y_pred = [p for p in predictions]
        with open('./{}.pkl'.format(FLAGS.model_dir.split('/')[-1]), 'wb') as f:
