...
from core_layers import MaskedConv2D

GlobalParams = collections.namedtuple('GlobalParams', [
    'batch_norm_momentum', 'batch_norm_epsilon', 'dropout_rate', 'data_format',
    'num_classes', 'depth_multiplier', 'depth_divisor', 'min_depth', 'mask',
])
...
BlockArgs = collections.namedtuple('BlockArgs', [
    ...
    'mid_filters', 'id_skip', 'strides', 'se_ratio'
])
...
class MnasBlock(object):
  ...
  def __init__(self, block_args, global_params):
    ...
    if global_params.mask:
        self.ConvFunc = MaskedConv2D
    else:
        self.ConvFunc = tf.keras.layers.Conv2D
  ...
  def _build(self):
    ...
    filters = self._block_args.mid_filters
    if self._block_args.input_filters != self._block_args.mid_filters:
      # Expansion phase:
      self._expand_conv = self.ConvFunc(
          filters,
          kernel_size=[1, 1],
          strides=[1, 1],
    ...
    self._project_conv = self.ConvFunc(
        filters,
        kernel_size=[1, 1],
        strides=[1, 1],
        kernel_initializer=conv_kernel_initializer,
...
class LeGRNetModel(tf.keras.Model):
  ...
  def __init__(self, blocks_args=None, global_params=None):
    ...
    """
    super(LeGRNetModel, self).__init__()
  ...
  def _build(self):
    ...
    for block_args in self._blocks_args:
      ...
      block_args = block_args._replace(
          input_filters=round_filters(block_args.input_filters,
                                      self._global_params),
          mid_filters=round_filters(block_args.mid_filters,
                                      self._global_params),
    ...
    if self._global_params.mask:
        ConvFunc = MaskedConv2D
    else:
        ConvFunc = tf.keras.layers.Conv2D

    # Stem part.
    self._conv_stem = ConvFunc(
        filters=round_filters(self._global_params.stem_filters, self._global_params),
        kernel_size=[3, 3],
        strides=[2, 2],
        kernel_initializer=conv_kernel_initializer,
        padding='same',
    ...
    # Head part.
    self._conv_head = ConvFunc(
        filters=self._global_params.last_filters,
        kernel_size=[1, 1],
        strides=[1, 1],
        kernel_initializer=conv_kernel_initializer,
        padding='same',
  ...
  def call(self, inputs, training=True):
    ...
    self.endpoints = {}
    # Calls Stem layers
    with tf.variable_scope('legr_stem'):
      outputs = tf.nn.relu(
    ...
    for idx, block in enumerate(self._blocks):
      with tf.variable_scope('legr_blocks_%s' % idx):
    ...
    with tf.variable_scope('legr_head'):
      outputs = tf.nn.relu(
